...
func emit(genCtx GenerationContext, outFolder string, generatedFileName string, templates []string, formatCode bool) error {
	if t, err := loadTemplate(templates...); err == nil {
		...
		if pkgTemplate, err := templateutil.Parse(t, getTemplateUtilityFunctions(genCtx.Model)); err != nil {
			return err
		} else {
			if err := templateutil.ProcessWrite2File(pkgTemplate, genCtx, destinationFile, formatCode); err != nil {
				return err
			}
		}
	} else {
...
func loadTemplate(templatePath ...string) ([]templateutil.Info, error) {
	...
	for _, tpath := range templatePath {
		...
		res = append(res, templateutil.Info{Name: filepath.Base(tpath), Content: string(b)})
	}
