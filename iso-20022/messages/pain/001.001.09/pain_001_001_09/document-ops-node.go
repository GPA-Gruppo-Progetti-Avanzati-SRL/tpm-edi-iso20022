...
func copy2DestNode(docPath string, dest, src interface{}, options *SetOpOptions) error {
	...
	const semLogContext = "pain_001_001_09::copy-to-dest"
	...
	case **CreditorReferenceType2:
		...
		return err
	case **CustomerCreditTransferInitiationV09:
		if typedStruct, ok := src.(CustomerCreditTransferInitiationV09); ok {
			*typedDest = &typedStruct
		} else {
			err = fmt.Errorf("value of type %T cannot be assigned", src)
		}
	...
	case **EquivalentAmount2:
		...
		return err
	case **ExchangeRate1:
		if typedStruct, ok := src.(ExchangeRate1); ok {
			*typedDest = &typedStruct
		} else {
			err = fmt.Errorf("value of type %T cannot be assigned", src)
		}
	...
	case **GenericIdentification30:
		...
		return err
	case **GroupHeader85:
		if typedStruct, ok := src.(GroupHeader85); ok {
			*typedDest = &typedStruct
		} else {
			err = fmt.Errorf("value of type %T cannot be assigned", src)
		}
	...
	case *[]Authorisation1Choice:
		...
		return err
	case *CreditTransferTransaction34:
		if typedStruct, ok := src.(CreditTransferTransaction34); ok {
			*typedDest = typedStruct
		} else {
			err = fmt.Errorf("value of type %T cannot be assigned", src)
		}
	...
	case *[]OtherContact1:
		...
		return err
	case *PaymentInstruction30:
		if typedStruct, ok := src.(PaymentInstruction30); ok {
			*typedDest = typedStruct
		} else {
			err = fmt.Errorf("value of type %T cannot be assigned", src)
		}
...
func (d *Document) ClearNode(path string, opts ...SetOpOption) error {
	const semLogContext = "pain_001_001_09::clear-node"
