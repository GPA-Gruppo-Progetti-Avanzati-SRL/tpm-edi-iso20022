...
func (d *Document) ClearPath(p string, opts ...SetOpOption) error {
	...
	const semLogContext = "pain_008_001_02::clear-path"
...
func (d *Document) GetProperty(path string, opts ...GetOpOption) (interface{}, error) {
	...
	const semLogContext = "pain_008_001_02::get-property"
...
func copy2Dest(docPath string, dest, src interface{}, options *SetOpOptions) error {
	...
	case *common.Authorisation1Code:
		...
		if options.isClearOperation {
			*typedDest = common.Authorisation1CodeZero
		} else {
	...
	case *common.Frequency1Code:
		...
		if options.isClearOperation {
			*typedDest = common.Frequency1CodeZero
		} else {
	...
	case *common.PaymentMethod2Code:
		if typedDest == nil {
			err = errors.New("nil pointer... in unmarshalling common.PaymentMethod2Code data for" + string(docPath))
			if err != nil && options.logVerbose {
				log.Error().Err(err).Str("path", string(docPath)).Interface("value", src).Msg(semLogContext)
			}
			return err
		...
		if options.isClearOperation {
			*typedDest = common.PaymentMethod2CodeZero
		} else {
	...
	case *common.SequenceType1Code:
		...
		if options.isClearOperation {
			*typedDest = common.SequenceType1CodeZero
		} else {
